        ...
        for root, dirs, files in os.walk(catkin_ws_path, followlinks=True):
            for currfile in files:
                if currfile.endswith(".stl") or currfile.endswith(".dae"):
                    ...
                    filename_path = os.path.join(root, currfile).replace(
                        catkin_stack_path, ""
                    )
            ...
            for dirpath, dirs, files in os.walk(models_path, followlinks=True):
                ...
                for currfile in files:
                    ...
                    try:
                        ...
                        print(
                            "Error parsing SDF file %s (%s). Ignoring model and continuing."
                            % (filename_path, e)
                        )
                        continue
                    root = tree.getroot()
                    ...
                    if modelname_in_file not in find_model_in_gazebo_dir.cache:
                        ...
                        find_model_in_gazebo_dir.cache[modelname_in_file] = (
                            filename_path
                        )
...
def pose2origin(node, pose):
    ...
    ET.SubElement(
        node,
        "origin",
        {"xyz": array2string(rounded(xyz)), "rpy": array2string(rounded(rpy))},
    )
...
class World(object):
    ...
    def from_tree(self, node, **kwargs):
        ...
        if node.findall("world"):
            ...
            # TODO lights
        self.models += [
            Model(tree=model_node, version=self.version)
    ...
    def plot(self, graph, prefix=""):
        ...
        for model in self.models:
            ...
            graph.add_edge(
                "world",
                (
                    model.name + "::" + model.root_link.name
                    if prefix is not None
                    else model.root_link.name
                ),
            )
...
class Model(SpatialEntity):
    ...
    def from_tree(self, node, **kwargs):
        ...
        self.joints = [
            Joint(self, tree=joint_node) for joint_node in node.findall("joint")
        ]
    ...
    def get_link(self, requested_linkname, prefix=""):
        ...
        for submodel in self.submodels:
            res = submodel.get_link(
                requested_linkname,
                prefix + "::" + submodel.name if prefix else submodel.name,
            )
            if res:
                return res
    ...
    def build_tree(self):
        for joint in self.joints:
            ...
            if not joint.tree_parent_link:
                print(
                    "Failed to find parent %s of joint %s. Aborting"
            ...
            if not joint.tree_child_link:
                print(
                    "Failed to find child %s of joint %s. Aborting"
    ...
    def calculate_absolute_pose(self, worldMVparent=identity_matrix()):
        ...
        for joint in self.joints:
            joint.pose_world = concatenate_matrices(
    ...
    def get_parent(self, requested_linkname, prefix="", visited=[]):
        ...
        if self.parent_model:
            return self.parent_model.get_parent(
                requested_linkname, self.name, visited + [self.name]
            )
...
class Link(SpatialEntity):
    ...
    def from_tree(self, node):
        ...
        self.collisions = [
            Collision(tree=collision_node)
            for collision_node in node.findall("collision")
        ]
        self.visuals = [
            Visual(tree=visual_node) for visual_node in node.findall("visual")
        ]
    ...
    def add_urdf_elements(self, node, prefix):
        ...
        if self.tree_parent_joint:
            if self.tree_parent_joint.parent_model == self.parent_model:
                urdf_pose = concatenate_matrices(
                    inverse_matrix(self.tree_parent_joint.pose_world), self.pose_world
                )
...
class Joint(SpatialEntity):
    ...
    def add_urdf_elements(self, node, prefix):
        ...
        parentnode = ET.SubElement(
            jointnode, "parent", {"link": sdf2tfname(full_prefix + self.parent)}
        )
        ...
        parent_pose_world = (
            self.tree_parent_link.tree_parent_joint.pose_world
            if self.tree_parent_link.tree_parent_joint
        ...
        if self.tree_parent_link.parent_model == self.parent_model:
            ...
            pose2origin(
                jointnode,
                concatenate_matrices(
                    inverse_matrix(parent_pose_world), self.tree_child_link.pose_world
                ),
            )
        if (
            self.type == "revolute"
            and self.axis.lower_limit == 0
            and self.axis.upper_limit == 0
        ):
            ...
            ET.SubElement(
                dummyjointnode, "parent", {"link": dummylinknode.attrib["name"]}
            )
            ET.SubElement(
                dummyjointnode, "child", {"link": sdf2tfname(full_prefix + self.child)}
            )
...
class Axis(object):
    ...
    def __repr__(self):
        return (
            "Axis(xyz=%s,%s lower_limit=%s, upper_limit=%s, effort=%s, velocity=%s)"
            % (
                self.xyz,
                (
                    " use_parent_model_frame=%s," % self.use_parent_model_frame
                    if self.version >= 1.5
                    else ""
                ),
                self.lower_limit,
    ...
    def add_urdf_elements(self, node, modelCBTjoint):
        if (self.version <= 1.4) or (
            self.version >= 1.5 and self.use_parent_model_frame
            ...
            if xyz_norm != 0.0:
                ...
                print(
                    "Error calculating axis of joint %s for given xyz=%s"
...
class LinkPart(SpatialEntity):
    ...
    def from_tree(self, node):
        ...
        if node.tag != "visual" and node.tag != "collision":
            print(
                "Invalid node of type %s instead of visual or collision. Aborting."
                % node.tag
            )
            return
            ...
            if typenode != None:
                ...
                if gtype == "box":
                    ...
                    self.geometry_data = {
                        ...
                        "length": get_tag(typenode, "length"),
                    }
                elif gtype == "sphere":
                    self.geometry_data = {"radius": get_tag(typenode, "radius")}
                elif gtype == "mesh":
                    self.geometry_data = {
                        "uri": get_tag(typenode, "uri"),
    ...
    def add_urdf_elements(self, node, prefix, link_pose, part_type):
        ...
        if self.geometry_type == "box":
            boxnode = ET.SubElement(
                geometrynode, "box", {"size": self.geometry_data["size"]}
            )
            ...
            spherenode = ET.SubElement(
                geometrynode, "sphere", {"radius": self.geometry_data["radius"]}
            )
