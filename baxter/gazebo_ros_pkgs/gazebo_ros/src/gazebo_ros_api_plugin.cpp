...
namespace gazebo
{
  ...
  GazeboRosApiPlugin::GazeboRosApiPlugin() : physics_reconfigure_initialized_(false),
...
#if GAZEBO_MAJOR_VERSION >= 8
        if (!world_->ModelByName(req.model_name))
          break;
#else
        if (!world_->GetModel(req.model_name))
          break;
#endif
      }
                                              ...
                                              gazebo_msgs::GetModelProperties::Response &res)
  {
    ...
    else
    {
      ...
      if (parent_model)
        res.parent_model_name = parent_model->GetName();
  ...
  void GazeboRosApiPlugin::physicsReconfigureCallback(gazebo_ros::PhysicsConfig &config, uint32_t level)
  {
    ...
    else
    {
      ...
      if (config.gravity_z != srv.response.gravity.z)
        changed = true;
      if (config.auto_disable_bodies != srv.response.ode_config.auto_disable_bodies)
        changed = true;
      if ((uint32_t)config.sor_pgs_precon_iters != srv.response.ode_config.sor_pgs_precon_iters)
        changed = true;
      if ((uint32_t)config.sor_pgs_iters != srv.response.ode_config.sor_pgs_iters)
        changed = true;
      if (config.sor_pgs_rms_error_tol != srv.response.ode_config.sor_pgs_rms_error_tol)
        changed = true;
      if (config.sor_pgs_w != srv.response.ode_config.sor_pgs_w)
        changed = true;
      if (config.contact_surface_layer != srv.response.ode_config.contact_surface_layer)
        changed = true;
      if (config.contact_max_correcting_vel != srv.response.ode_config.contact_max_correcting_vel)
        changed = true;
                                               ...
                                               const ignition::math::Quaterniond &initial_q)
  {
    ...
    if (model_tixml)
    {
      ...
      ignition::math::Pose3d model_pose = ignition::math::Pose3d(xyz, ignition::math::Quaterniond(rpy)) + ignition::math::Pose3d(initial_xyz, initial_q);
                                           ...
                                           gazebo_msgs::SpawnModel::Response &res)
  {
    ...
    while (ros::ok())
    {
...

      {
        // boost::recursive_mutex::scoped_lock lock(*world->GetMRMutex());
#if GAZEBO_MAJOR_VERSION >= 8
        if ((isLight && world_->LightByName(model_name) != NULL) || (world_->ModelByName(model_name) != NULL))
#else
        if ((isLight && world_->Light(model_name) != NULL) || (world_->GetModel(model_name) != NULL))
#endif
          break;
      }
